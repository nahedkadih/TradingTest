...
namespace ConsoleNahedTest
{
    public static class TradingEngineDictionary4
    {
        ...
        public static SortedDictionary<long, Order>[] BuyOrdersBook  = new SortedDictionary<long, Order>[Global.MAX_PRICE + 1];
        public static SortedDictionary<long, Order>[] SellOrdersBook = new SortedDictionary<long, Order>[Global.MAX_PRICE + 1];
                    ...
                    for (int i = 0; i < _orders.Count; i++)
                    {
                       ...
                       Global. PrintToScreen(SellOrder.OrderId, SellOrder.Price, minQuantity, order.OrderId, order.Price, minQuantity);
                        if (order.Quantity >= SellOrder.Quantity)
                        {
                            ...
                            _orders.Remove(SellOrder.TimeTicks);
                            i--;
        ...
        public static void ModifyOrder(string orderId, string orderSide, ushort newPrice, int newQty)
        {
            ...
            {
                SortedDictionary<long, Order>[] _currentBook = null; 
                SortedDictionary<long, Order> _orders;
        ...
        public static void Print()
        {
            ...
            for (int priceEntry = BuyOrdersBook.Count() - 1; priceEntry >= 0; priceEntry--)
            {
                ...
                var asks_sum = SellOrdersBook[priceEntry].Sum(x => x.Value.Quantity);
            ...
            for (int priceEntry = BuyOrdersBook.Count() - 1; priceEntry >= 0; priceEntry--)
            {
                ...
                var asks_sum = BuyOrdersBook[priceEntry].Sum(x => x.Value.Quantity);
                Global.PrintLine($"{priceEntry} {asks_sum}");
        ...
        public static void Reset()
        {
            ...
            for (int size = Global.MAX_PRICE; size >= 0; size--)
                BuyOrdersBook[size] = new   SortedDictionary<long, Order>();

            for (int size = Global.MAX_PRICE; size >= 0; size--)
                SellOrdersBook[size] = new SortedDictionary<long, Order>();
